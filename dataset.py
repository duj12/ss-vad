...
class TrainHDF5Dataset(tdata.Dataset):
    ...
    def __init__(self,
        ...
        self._txthardlabel = txthardlabel
        self._datasetcache = {}
        self._labelcache = {}
        self._hardlabelcache = {}
    ...
    def __getitem__(self, index: int):
        ...
        #add hard label if given
        target_hard_1d = hard_txt_label.strip().split()
        target_hard_1d = torch.tensor([int(x) for x in target_hard_1d])

        target_hard = torch.zeros((target_hard_1d.size(0), 2))
        target_hard[:,0] = 1-target_hard_1d
        target_hard[:,1] = target_hard_1d
        #in case of different frame rate between feats and hard labels, sample the hard label
        hyp_sz = data.size(0)
        ref_sz = target_hard.size(0)
...
def sequential_collate(batches):
    # sort length wise
    data, targets_time, targets_clip, targets_hard, fnames = zip(*batches)
    data, lengths_data = pad(data)
    targets_time, lengths_tar = pad(targets_time, padding_value=0)
    targets_clip = torch.stack(targets_clip)
    #targets_hard = [x.unsqueeze(1) for x in targets_hard]  #for shape (batch,) need one more dimension
    targets_hard, _ = pad(targets_hard, padding_value=0)
    #targets_hard = targets_hard.squeeze(1)
    assert lengths_data.shape == lengths_tar.shape
    return data, targets_time, targets_clip, targets_hard, fnames, lengths_tar
